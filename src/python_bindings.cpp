...
PYBIND11_MODULE(min_alpha_zero, m) {
  ...
  py::class_<alphazero::Node>(m, "Node")
      ...
      .def("get_child_node", &alphazero::Node::getChildNode,
           py::return_value_policy::reference)
      .def_property_readonly("child_visits",
                             [](const alphazero::Node &n) {
                               return py::array_t<int>(
                                   {alphazero::kNumActions}, {sizeof(int)},
                                   n.child_visits.data(), py::cast(n));
                             })
      .def_property_readonly("child_values",
                             [](const alphazero::Node &n) {
                               return py::array_t<float>(
                                   {alphazero::kNumActions}, {sizeof(float)},
                                   n.child_values.data(), py::cast(n));
                             })
      .def_property_readonly("legal_mask",
                             [](const alphazero::Node &n) {
                               return py::array_t<bool>(
                                   {alphazero::kNumActions}, {sizeof(bool)},
                                   n.legal_mask.data(), py::cast(n));
                             })
      .def_property_readonly("policy", [](const alphazero::Node &n) {
        return py::array_t<float>({alphazero::kNumActions}, {sizeof(float)},
                                  n.policy.data(), py::cast(n));
      });
  ...
  py::class_<alphazero::Game>(m, "Game")
      ...
      .def(
          "get_root",
          [](alphazero::Game &g) -> alphazero::Node * { return g.root.get(); },
          py::return_value_policy::reference);
  ...
  py::class_<alphazero::RawModelAgent, alphazero::ChessAgent>(m,
                                                              "RawModelAgent")
      .def(py::init<>())
      .def("select_action", &alphazero::RawModelAgent::select_action)
      .def("name", &alphazero::RawModelAgent::name);
  ...
  m.def("dump_game_tree_to_file", &alphazero::dump_game_tree_to_file,
        py::arg("game"), py::arg("g") = 0, py::arg("m") = 0,
        py::arg("chosen_action") = -1);
  ...
  m.def("start_evaluator_thread", []() {
    if (!g_evaluator_thread) {
      g_evaluator_thread =
          std::make_unique<std::thread>(alphazero::run_evaluator);
    }
  });
  ...
  m.def(
      "get_config", []() -> alphazero::Config & { return alphazero::g_config; },
      py::return_value_policy::reference);
}