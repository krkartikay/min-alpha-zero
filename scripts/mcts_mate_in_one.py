...
def print_game_tree(node, depth=0, max_depth=3, action=-1):
    ...
    print(
        f"{indent}Node{history_str}: visits={visits_total}, value={node.value:.3f}, evaluated={node.is_evaluated}, leaf={node.is_leaf}"
    )
...
def main():
    ...
    config.model_path = (
        sys.argv[1] if len(sys.argv) == 2 and sys.argv[1] else "model.pt"
    )
    config.debug = False
