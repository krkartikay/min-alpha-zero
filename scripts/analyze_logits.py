...
def analyze_model_logits(model_path):
    ...
    board_tensor = board_to_tensor(board).unsqueeze(0).to(device)  # Add batch dimension and move to device
    
    with torch.no_grad():
        policy_logits, value = model(board_tensor)
        policy_logits = policy_logits.squeeze(0).cpu()  # Remove batch dimension and move to CPU
...
def plot_logits_comparison():
    ...
    for i, result in enumerate(results):
        ...
        axes[0, i].hist(result['all_logits'], bins=50, alpha=0.7, color='blue', label='All actions')
        axes[0, i].axvline(result['mate_logit'], color='red', linestyle='--', linewidth=2, label=f'Mate move: {result["mate_logit"]:.3f}')
        ...
        axes[1, i].hist(result['legal_logits'], bins=20, alpha=0.7, color='green', label='Legal actions')
        axes[1, i].axvline(result['mate_logit'], color='red', linestyle='--', linewidth=2, label=f'Mate move: {result["mate_logit"]:.3f}')
        ...
        # Print stats
        print(f"\n{model_name}:")
        print(f"  All logits - mean: {np.mean(result['all_logits']):.3f}, std: {np.std(result['all_logits']):.3f}")
        print(f"  Legal logits - mean: {np.mean(result['legal_logits']):.3f}, std: {np.std(result['legal_logits']):.3f}")
        print(f"  Mate logit: {result['mate_logit']:.6f}")
        print(f"  Mate rank among legal: {np.sum(result['legal_logits'] > result['mate_logit']) + 1}/{len(result['legal_logits'])}")
    ...
    for i, result in enumerate(results):
        ...
        mate_idx = np.where(np.array(result['legal_actions']) == result['mate_action'])[0]
        mate_prob = legal_probs[mate_idx[0]] if len(mate_idx) > 0 else 0
        
        axes2[i].hist(legal_probs, bins=20, alpha=0.7, color='purple', label='Legal move probs')
        axes2[i].axvline(mate_prob, color='red', linestyle='--', linewidth=2, label=f'Mate prob: {mate_prob:.6f}')
