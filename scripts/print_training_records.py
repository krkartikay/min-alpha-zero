...
def action_to_san(action, board):
    ...
    try:
        ...
        if piece and piece.piece_type == chess.PAWN:
            if (piece.color == chess.WHITE and to_sq >= 56) or (piece.color == chess.BLACK and to_sq <= 7):
    ...
    for i in range(start_record, end_record):
        ...
        try:
            ...
            if len(legal_indices) > 0:
                legal_policies = [(j, policy[j].item()) for j in legal_indices]
                legal_policies.sort(key=lambda x: x[1], reverse=True)
                
                print("Top 3 moves by policy:")
                for rank, (action, prob) in enumerate(legal_policies[:3]):
                    san_move = action_to_san(action, board)
                    visits_count = visits[action].item()
                    print(f"  {rank+1}. {san_move}: policy={prob:.6f}, visits={visits_count}")
            else:
